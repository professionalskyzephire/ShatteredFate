...
namespace ShatteredFate.Content.Projectiles.Misc
{
    public class FallingCosmicDust : ModProjectile
    {
        ...
        public override void AI() {
            int d = Dust.NewDust(Projectile.position, Projectile.width, Projectile.height, DustID.FireworksRGB, 0f, 0f, 0, Color.Cyan, 0.5f);
            Main.dust[d].velocity += Projectile.velocity.RotatedBy(MathHelper.PiOver2) * 0.2f;
            Main.dust[d].position += Main.dust[d].velocity;
            Main.dust[d].noGravity = true;
            d = Dust.NewDust(Projectile.position, Projectile.width, Projectile.height, DustID.FireworksRGB, 0f, 0f, 0, Color.Pink, 0.5f);
            Main.dust[d].velocity -= Projectile.velocity.RotatedBy(MathHelper.PiOver2) * 0.2f;
            Main.dust[d].position += Main.dust[d].velocity;
            Main.dust[d].noGravity = true;
        ...
        public override void OnKill(int timeLeft) {
            NetMessage.SendData(MessageID.SyncItem, -1, -1, null, Item.NewItem(Projectile.GetSource_Death(), (int)Projectile.Center.X, (int)Projectile.Center.Y, 0, 0, ModContent.ItemType<Content.Items.Materials.CosmicDust>(), 1, false, 0, false, false));
        }
        ...
        public override bool PreDraw(ref Color lightColor) {
            ...
            for (int i = 1; i < Projectile.oldPos.Length; i++) Main.EntitySpriteDraw(texture, Projectile.oldPos[i] + Projectile.Size / 2 - Main.screenPosition, null, Color.Lerp(new Color(0, 255, 255, 0), new Color(255, 0, 255, 0), i / (float)Projectile.oldPos.Length) * MathHelper.Lerp(1f, 0f, i / (float)Projectile.oldPos.Length), Projectile.oldRot[i] + MathHelper.PiOver2, texture.Size() / 2, Projectile.scale * new Vector2(MathHelper.Lerp(1.6f, 0.05f, i / (float)Projectile.oldPos.Length), i < 3 ? 1.6f - (3 - i) * 0.4f : 1.6f), SpriteEffects.None, 0);
            texture = (Texture2D)ModContent.Request<Texture2D>(Texture);
            for (int i = 1; i < Projectile.oldPos.Length; i++) Main.EntitySpriteDraw(texture, Projectile.oldPos[i] + Projectile.Size / 2 - Main.screenPosition, null, Color.White with { A = 0 } * MathHelper.Lerp(0.75f, 0f, i / (float)Projectile.oldPos.Length), Projectile.rotation - MathHelper.PiOver2, texture.Size() / 2, Projectile.scale * new Vector2(MathHelper.Lerp(1f, 0.3f, i / (float)Projectile.oldPos.Length), 1f), SpriteEffects.None, 0);
