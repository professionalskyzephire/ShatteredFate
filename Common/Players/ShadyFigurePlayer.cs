...
public class ShadyFigurePlayer : ModPlayer {
    ...
    public override void LoadData(TagCompound tag) => SetShadySpawnStatus(tag.GetBool($"{SFMod.ModName}:ShadySpawned"));
    public override void SaveData(TagCompound tag) => tag[$"{SFMod.ModName}:ShadySpawned"] = GetShadySpawnStatus();
    public override void PostUpdate() {
        if (!Main.dayTime && !GetShadySpawnStatus()) {
            ...
            if (Main.netMode == NetmodeID.Server && index < Main.maxNPCs) { NetMessage.SendData(MessageID.SyncNPC, number: index); };
